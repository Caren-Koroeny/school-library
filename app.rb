...
class App
  ...
  def list_rentals(people)
    ...
    person = find_person_by_id(people, person_id)
  ...
  def find_person_by_id(people, person_id)
    people.find { |person| person.id == person_id }
  end

  def display_rentals(person)
    if person.rentals.empty?
      puts "#{person.name} didn't rent any book yet\n\n"
    else
      puts "Rentals for #{person.name}:"
      rentals = person.rentals.uniq # Remove duplicates from rentals list
